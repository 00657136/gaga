...
class getyoutubePlaylistData: ObservableObject {
    ...
    init(){
        let url = "https://www.googleapis.com/youtube/v3/playlistItems?part=snippet,contentDetails,status&playlistId=UUNL1ZadSjHpjm4q9j2sVtOA&key=AIzaSyCaTagZL_ssplF9SYM6Ifb3MSVgYkeuADY&maxResults=50"
...
class getyoutubeChannelData: ObservableObject {
    ...
    init(){
        let url = "https://www.googleapis.com/youtube/v3/channels?part=brandingSettings,snippet,contentDetails,statistics,status&id=UCNL1ZadSjHpjm4q9j2sVtOA&key=AIzaSyCaTagZL_ssplF9SYM6Ifb3MSVgYkeuADY"
        let session = URLSession(configuration: .default)
        session.dataTask(with: URL(string: url)!){(data, _, err) in
            if err != nil{
                print((err?.localizedDescription)!)
